...
### RISK-009 — Kira findings must include actionable recommended_change
**Status:** Open
Kira (and admin_merge) findings that reach the editor without a non-empty `recommended_change` are silently shown as non-actionable (no inline suggestion, no accept button). A finding with a vague or missing `recommended_change` is a quality failure, not a UI edge case.
**Acceptance:** The Kira review loop must always emit a concrete `recommended_change` containing the full replacement clause text. Findings without an actionable `recommended_change` must be either regenerated by the Kira loop, marked `non_actionable`, or at minimum flagged in the stage output so reviewers know no replacement text was produced. No finding should silently reach `awaiting_human_review` with a null or generic `recommended_change`.
