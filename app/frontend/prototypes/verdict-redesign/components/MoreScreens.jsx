...
function HistoryScreen({ navigate }) {
  ...
  const filtered = filter === "all" ? MOCK_HISTORY : MOCK_HISTORY.filter(e => e.state === filter);
  const approved = MOCK_HISTORY.filter(e => e.action === "approved").length;
  const rejected = MOCK_HISTORY.filter(e => e.action === "rejected").length;
    ...
    <div style={{ paddingTop: "var(--density-pad)", paddingBottom: 80, animation: "fadeIn 0.35s ease both" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40 }}>
        ...
        <div style={{ display: "flex", gap: 40, alignItems: "flex-end" }}>
          {[
            { label: "Total Runs", value: MOCK_HISTORY.length, color: null },
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--border)" }}>
        {filterDefs.map(f => {
          const count = f.key === "all" ? MOCK_HISTORY.length : MOCK_HISTORY.filter(e => e.state === f.key).length;
          ...
          return (
            <button key={f.key} onClick={() => setFilter(f.key)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px", fontSize: 13, fontWeight: 500, color: filter === f.key ? "var(--text)" : "var(--text-2)", background: "none", border: "none", borderBottom: filter === f.key ? "2px solid var(--text)" : "2px solid transparent", marginBottom: -1, cursor: "pointer", transition: "all 0.15s" }}>
              {f.label}
              <span style={{ fontSize: 10.5, color: "var(--text-3)" }}>{count}</span>
            </button>
          );
        })}
      </div>
...
function ProfileScreen() {
  const [user, setUser]           = React.useState({ ...MOCK_USER_PROFILE });
  const [avatarColor, setColor]   = React.useState(MOCK_USER_PROFILE.avatarColor);
  ...
  function save() {
    setSaved("saving");
    setTimeout(() => { setSaved("saved"); setTimeout(() => setSaved(null), 2200); }, 700);
  }
      ...
      <div>
        ...
        {section === "password" && (
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            <SaveButton state={savedState} onSave={save} label="Update Password" />
          </div>
        )}
        ...
        {section === "workspaces" && (
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            {MOCK_PROFILE_WORKSPACES.map((ws, i) => (
              <div key={ws.id} style={{ display: "grid", gridTemplateColumns: "1fr 100px 80px", gap: "0 16px", padding: "15px 0", borderBottom: "1px solid var(--border-light)", alignItems: "center", animation: `fadeIn 0.3s ${i * 0.07}s both` }}>
                <div>
