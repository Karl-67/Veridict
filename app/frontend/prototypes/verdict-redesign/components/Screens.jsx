...
function ContractDetailScreen({ navigate, selectedContractId, onViewRun, onAddVersion }) {
  const detail = MOCK_CONTRACT_DETAIL[selectedContractId] ?? MOCK_CONTRACT_DETAIL[1];
    ...
    <div style={{ paddingTop: "var(--density-pad)", paddingBottom: 80, animation: "fadeIn 0.35s ease both" }}>
      ...
      {uploadOpen && (
        <div style={{ border: "1px dashed var(--border)", borderRadius: 8, padding: "28px 28px", marginBottom: 40, animation: "fadeIn 0.2s ease", display: "flex", gap: 24, alignItems: "center" }}>
          ...
          <button onClick={() => { setUploadOpen(false); onAddVersion(); }} style={{ padding: "8px 18px", borderRadius: 7, background: "var(--text)", color: "var(--bg)", fontSize: 13, fontWeight: 600, border: "none", cursor: "pointer", flexShrink: 0 }}>Upload &amp; Analyze</button>
          ...
          return (
            <div key={v.id} style={{ display: "grid", gridTemplateColumns: "72px 1fr 144px 96px 120px 100px", gap: "0 16px", padding: "17px 0", borderBottom: "1px solid var(--border-light)", animation: `fadeIn 0.3s ${i * 0.07}s both`, alignItems: "center" }}>
              ...
              <div style={{ textAlign: "right" }}>
                {v.runId && (
                  <button onClick={onViewRun} style={{ fontSize: 12, fontWeight: 500, color: "var(--accent)", textDecoration: "underline", textUnderlineOffset: 3, border: "none", background: "none", cursor: "pointer" }}>View Report →</button>
                )}
              </div>
            </div>
          );
...
function ProcessingScreen({ navigate, fileName, processingStage }) {
