...
function VerdictScreen({ navigate, selectedContractId, onApprove, onReject }) {
  const verdict = MOCK_VERDICT;
  const fileName = "NDA — TechVentures LLC.pdf";
  const risk = verdictRiskConfig[verdict.riskLevel];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 80, alignItems: "start" }}>
        ...
        <div style={{ position: "sticky", top: 72, display: "flex", flexDirection: "column", gap: 40 }}>
            ...
            <div style={{ border: "1px solid var(--border)", borderRadius: 10, padding: 24, background: "var(--surface)" }}>
              ...
              {!rejectMode ? (
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={() => { setApproved(true); onApprove && setTimeout(onApprove, 800); }} style={{ flex: 1, padding: "9px 14px", borderRadius: 7, background: "var(--accent)", color: "white", fontSize: 13, fontWeight: 600, border: "none", cursor: "pointer", transition: "opacity 0.15s" }}
                ...
                <div style={{ animation: "fadeIn 0.2s ease" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => { onReject && onReject(rejectReason); }} style={{ flex: 1, padding: "8px", borderRadius: 7, background: "var(--risk-high)", color: "white", fontSize: 13, fontWeight: 600, border: "none", cursor: "pointer" }}>Confirm</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div>
              {[
                { label: "Generate Redline",       desc: "AI-drafted revision with tracked changes", accent: true  },
                { label: "Schedule Partner Review", desc: "Book a review session with your team",     accent: false },
                { label: "Export Report",           desc: "Download full analysis as PDF",            accent: false },
              ].map(({ label, desc, accent }) => (
                <button key={label} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "12px 0", borderBottom: "1px solid var(--border-light)", textAlign: "left", background: "none", border: "none", borderBottom: "1px solid var(--border-light)", cursor: "pointer", transition: "padding-left 0.15s" }}
